...
def find_color_card(rgb_img, threshold_type='adaptgauss', threshvalue=125, blurry=False, background='dark'):
    ...
    df = df[['index', 'X', 'Y', 'width', 'height', 'WHratio', 'Area', 'square', 'child','blurriness', 'distprox', 'sizeprox']]
...
def quick_color_check(target_matrix, source_matrix, num_chips):
    ...
    dataset = pd.DataFrame({'source': all_color_data[:, 0], 'target': all_color_data[:, 1], 'color': all_color_data[:, 2]})
...
def create_color_card_mask(rgb_img, radius, start_coord, spacing, nrows, ncols, exclude=[]):
    ...
    for chip in chips:
        #mask = cv2.drawContours(mask, chip[0], -1, (255, 255, 255), -1)
...
def get_color_matrix(rgb_img, mask):
    ...
    for i in np.unique(mask):
        if i != 0:
            ...
            color_matrix[row_counter][1] = np.median(chip[:, 0])
            color_matrix[row_counter][2] = np.median(chip[:, 1])
            color_matrix[row_counter][3] = np.median(chip[:, 2])
            row_counter += 1
...
def correct_color(target_img, target_mask, source_img, source_mask, output_directory):
    ...
    np.savez(os.path.join(output_directory, "target_matrix.npz"), target_matrix)
    #save_matrix(source_matrix, os.path.join(output_directory, "source_matrix.npz"))
    np.savez(os.path.join(output_directory, "source_matrix.npz"), source_matrix)
    # get matrix_m
    ...
    np.savez(os.path.join(output_directory, "transformation_matrix.npz"), transformation_matrix)
...
def get_matrix_m(target_matrix, source_matrix):
    ...
    Inputs:
    target_matrix       = a 22x4 matrix containing the average red value, average green value, and average blue value for each color chip.
    source_matrix       = a 22x4 matrix containing the average red value, average green value, and average blue value for each color chip.
    Outputs:
...
def orient_color_card(color_matrix, ncols):
    if ncols == 4:
        ## For each potential chip alculate variance across RGB channels
        ## If variance between RGB channels is low it is a grayscale chip
        chip1 = var(color_matrix[0, 1:])
        chip5 = var(color_matrix[4, 1:])
        chip9 = var(color_matrix[8, 1:])
        chip13 = var(color_matrix[12, 1:])
        chip17 = var(color_matrix[16, 1:])
        chip21 = var(color_matrix[20, 1:])
        if (chip1 < 100 and chip5 < 100 and chip9 < 100 and chip13 < 100 and chip17 < 100 and chip21 < 100):
            ...
            #print(color_matrix)
            chips = np.arange(10, 250,10).tolist()
            chips = chips[::-1]
            color_matrix[:,0] = chips
            color_matrix = color_matrix[color_matrix[:,0].argsort()]
            #print("Correct?")
            #print(color_matrix)
            return color_matrix
    elif ncols == 6:
        ## Calculate variance across RGB channels
        ## If variance between channels is low that is a grayscale chip
        chip1 = var(color_matrix[0, 1:])
        chip2 = var(color_matrix[1, 1:])
        chip3 = var(color_matrix[2, 1:])
        chip4 = var(color_matrix[3, 1:])
        chip5 = var(color_matrix[4, 1:])
        chip6 = var(color_matrix[5, 1:])
        if (chip1 < 100 and chip2 < 100 and chip3 < 100 and chip4 < 100 and chip5 < 100 and chip6 < 100):
            print("Horizontal - Standard orientation!\n")
            return color_matrix
        else:
            print("Horizontal - Opposite orientation!\n")
            chips = np.arange(10, 250,10).tolist()
            chips = chips[::-1]
            color_matrix[:,0] = chips
            color_matrix = color_matrix[color_matrix[:, 0].argsort()]
        ...
        print("This type of color checker is not supported...\n")
...
def rotate_color_card(df, spacing, img):
    df.sort_values(by=['X', 'Y'], ascending='True')
    df_xy = df[['X', 'Y']]
    cutoff = (spacing[1]) / 2
    y_max = max(df_xy['Y'])
    y_cutoff_max = y_max - cutoff
    y_min = min(df_xy['Y'])
    y_cutoff_min = y_min + cutoff
    row_xy_max = df[df['Y'] > y_cutoff_max]
    row_xy_min = df[df['Y'] < y_cutoff_min]
    slope_top = scipy.stats.linregress(row_xy_max['X'], row_xy_max['Y'])[0]
    slope_bottom = scipy.stats.linregress(row_xy_min['X'], row_xy_min['Y'])[0]
    slope = np.mean([slope_top, slope_bottom])
    rotation_deg = math.degrees(math.atan(0)) - math.degrees(math.atan(slope))
    output_img = scipy.ndimage.rotate(img, -rotation_deg, reshape=True)
    if slope > 1:
        rotation_deg = math.degrees(math.atan(0)) - math.degrees(math.atan(slope))
        output_img = scipy.ndimage.rotate(img, -rotation_deg, reshape=True)
    elif slope < 1:
